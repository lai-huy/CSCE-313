...
void worker_thread_function(int m, FIFORequestChannel* workerChannel, string fileName, BoundedBuffer* response_buffer, BoundedBuffer* request_buffer) {
	char* buf = new char[m];
...
int main(int argc, char* argv[]) {
	...
	vector<FIFORequestChannel*> FIFOVec;    // array of FIFOs (w elements)
	...
	for (int j = 0; j < w; j++) {
		MESSAGE_TYPE tempMsg = NEWCHANNEL_MSG;
		chan->cwrite(&tempMsg, sizeof(tempMsg));
		char newChannel[1024];
		chan->cread(newChannel, sizeof(newChannel));

		FIFORequestChannel* updatedChannel = new FIFORequestChannel(newChannel, FIFORequestChannel::CLIENT_SIDE);
		FIFOVec.push_back(updatedChannel);
		workerVec.push_back(thread(worker_thread_function, m, updatedChannel, f, &response_buffer, &request_buffer));
	}
	...
	for (size_t i = 0; i < FIFOVec.size(); ++i) {
	...
	for (FIFORequestChannel*& chan : FIFOVec) {
