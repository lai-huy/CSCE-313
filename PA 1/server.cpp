...
void process_file_request(FIFORequestChannel* rc, char* request) {
	...
	if (f.length > buffercapacity) {
		cerr << "Client is requesting a chunk bigger than server's capacity\n";
		cerr << "Returning nothing (i.e., 0 bytes) in response\n";
		rc->cwrite(response, 0);
	}

	FILE* fp = fopen(filename.c_str(), "rb");
	if (!fp) {
		cerr << "Server received request for file: " << filename << " which cannot be opened\n";
		rc->cwrite(buffer, 0);
		return;
	}
...
void handle_process_loop(FIFORequestChannel* channel) {
	...
	while (true) {
		...
		if (nbytes < 0) {
			cerr << "Client-side terminated abnormally\n";
			break;
		} else if (nbytes == 0) {
			cout << "Server could not read anything... Terminating\n";
			break;
		}
